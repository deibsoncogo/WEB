...
export function AuthWrapper({children}: any){
       ...
       const token = localStorage.getItem("access_token")          
        if(token){     
            ...
            if(parseInt(values.exp) < (Date.now() / 1000)){
                ...
                switch(values.role){
                    case roles.ADMIN: setHasPermission(true); return;
                    case roles.TEACHER: professorRoutes.includes(currentRoute)? setHasPermission(true):  route.push("/"); return;                     
                    default:
    ...
    if (rotas !== currentRoute || !rotas)
       setRotas(currentRoute)     
